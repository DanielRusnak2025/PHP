<img width="1432" height="727" alt="изображение" src="https://github.com/user-attachments/assets/b807f834-5993-4788-988a-31fd3093f6c7" />Шаг 1: Установка PHP


1.  https://www.php.net/downloads
2.  <img width="757" height="319" alt="изображение" src="https://github.com/user-attachments/assets/590fc754-58d8-49a8-90e9-6fc994351131" />

3.   <img width="669" height="246" alt="изображение" src="https://github.com/user-attachments/assets/6f7f2525-f871-4a20-839d-8936919270aa" />
4.   <img width="454" height="114" alt="изображение" src="https://github.com/user-attachments/assets/b4fd944b-eeda-4aab-a01a-d0f920b70772" />
5.   <img width="702" height="189" alt="изображение" src="https://github.com/user-attachments/assets/91eccead-4d71-4087-a0e1-b0aa26d573cf" />


_________________________________________________________________________________________________________________________________________________
Шаг 2: Альтернативный способ установки PHP


1. <img width="505" height="386" alt="изображение" src="https://github.com/user-attachments/assets/d264dd95-acf7-4cd2-ae34-e89d6078471f" />
2. <img width="497" height="416" alt="изображение" src="https://github.com/user-attachments/assets/9d6edb90-9920-499c-9597-2edece7c9aca" />
3. <img width="665" height="434" alt="изображение" src="https://github.com/user-attachments/assets/97b78c63-9984-4a5a-92a9-626daa1b7d79" />
4. <img width="1432" height="727" alt="изображение" src="https://github.com/user-attachments/assets/a9937184-763a-44ec-8f70-1de0fb30ab88" />


_________________________________________________________________________________________________________________________________________________
Шаг 3. Написание первой PHP-программы


1.  
2. <img width="497" height="106" alt="изображение" src="https://github.com/user-attachments/assets/4e4e93d2-807b-4ad7-ad2d-816d9bae95dd" />
3. <img width="284" height="122" alt="изображение" src="https://github.com/user-attachments/assets/14f1b2fd-6daf-4ae1-9a18-1b4cf1664481" />


_________________________________________________________________________________________________________________________________________________
первый метод (встроенный веб-сервер PHP) :
4.  
    <img width="719" height="130" alt="изображение" src="https://github.com/user-attachments/assets/49144fba-e9f6-4381-baad-a19b23b03b9c" />
    <img width="482" height="91" alt="изображение" src="https://github.com/user-attachments/assets/00073a26-adb3-4845-a45e-f951fd19ddc3" />

    
второй метод (XAMPP) :
4.  
    <img width="448" height="107" alt="изображение" src="https://github.com/user-attachments/assets/9904dde6-6bb0-4ad8-8ff8-1c274788f125" />
    <img width="457" height="98" alt="изображение" src="https://github.com/user-attachments/assets/b98cd7dc-984f-4873-8b00-5ab724a9d6ee" />


_________________________________________________________________________________________________________________________________________________
Шаг 4. Вывод данных в PHP
    
первый метод (встроенный веб-сервер PHP) :
<img width="417" height="88" alt="изображение" src="https://github.com/user-attachments/assets/b5c8ea7d-d186-4f2c-b7a0-d90346717d93" />

второй метод (XAMPP) :
<img width="442" height="83" alt="изображение" src="https://github.com/user-attachments/assets/7fb41af7-6bb8-4fe1-ae4f-91f890a42663" />


_________________________________________________________________________________________________________________________________________________
Шаг 5. Работа с переменными и выводом

<img width="619" height="189" alt="изображение" src="https://github.com/user-attachments/assets/e4e99de6-e6e1-4cc6-8a78-802687f43292" />
<img width="446" height="101" alt="изображение" src="https://github.com/user-attachments/assets/3b3d171a-79e9-400f-b3e5-d787b74d402d" />


_________________________________________________________________________________________________________________________________________________
Контрольные вопросы

1.    Ручная установка (zip-архив): 
       - Скачали PHP для Windows
       - Распаковали в папку
       - Добавили путь в PATH
       - Запускаем через php.exe

       Установка через XAMPP
       - PHP идёт вместе с Apache
       - Ничего не настраиваешь вручную
       - Сразу можно открывать PHP в браузере

2.    php -v

        PHP 8.5.3 (cli) (built: Feb 10 2026 18:43:53) (NTS Visual C++ 2022 x64)
        Copyright (c) The PHP Group
        Built by The PHP Group
        Zend Engine v4.5.3, Copyright (c) Zend Technologies
        with Zend OPcache v8.5.3, Copyright (c), by Zend Technologies

3.    echo

       - Языковая конструкция.
       - Может выводить несколько аргументов.
       - Работает чуть быстрее.
       - Ничего не возвращает.
       - echo "a", "b", "c";


      print

       - Тоже языковая конструкция.
       - Принимает только один аргумент.
       - Возвращает 1, поэтому можно использовать в выражениях.
       - Чуть медленнее, но разница почти нулевая.
       - $result = print "a"; // $result = 1
